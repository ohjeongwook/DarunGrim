	...
	for(;current_addr<=endEA;)
	{
		...
		*pFlag=getFlags(current_addr);
		...
		if(FoundBranching)
		{
			...
			//If Splitted Block
			//must be jmp,next block has only on cref_to
			if(cref_list.size()==1 && current_itype==NN_jmp)
			{
...
void AnalyzeIDADataByRegion(bool (*Callback)(PVOID context,BYTE type,PBYTE data,DWORD length),PVOID Context,list <AddressRegion> *pAddressRegions,int GatherCmdArray)
{
	...
	for(AddressRegionsIter=pAddressRegions->begin();AddressRegionsIter!=pAddressRegions->end();AddressRegionsIter++)
	{
		...
		ea_t current_addr;
		for(current_addr=startEA;current_addr<endEA;)
		{
			list <insn_t> CmdArray;
			flags_t Flag;
			
			ea_t new_addr=AnalyzeBlock(Callback,Context,current_addr,endEA,&CmdArray,&Flag,AdditionallyAnalyzedBlocks);
			if(0)
			{
				hash_map <op_t,OperandPosition,OpTHashCompareStr> OperandsHash;
				multimap <OperandPosition,OperandPosition,OperandPositionCompareTrait> InstructionMap;
				...
				if(NewCmdArray)
				{
					DumpOneLocationInfo(Callback,Context,current_addr,NewCmdArray,Flag,GatherCmdArray);
					delete NewCmdArray;
				}
			}else
			{
				DumpOneLocationInfo(Callback,Context,current_addr,&CmdArray,Flag,GatherCmdArray);
			}
			CmdArray.clear();
