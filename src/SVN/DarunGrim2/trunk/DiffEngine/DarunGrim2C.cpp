...
<database filename>\r\n\
	...
	if(RetrieveFromFile && TheSourceFilename && TheTargetFilename && OutputFilename)
	{
		...
		//Create IDC file
		char *EscapedOutputFilename=(char *)malloc(strlen(OutputFilename)*2+1);
		if(EscapedOutputFilename)
		{
			...
			char *IDCFilename=WriteToTemporaryFile("static main()\n\
				{\n\
					...
					Exit(0);\n\
					}",EscapedLogFilename?EscapedLogFilename:"",EscapedOutputFilename,
					StartAddressForTheOriginal,
					EndAddressForTheOriginal
				);

			if(IDCFilename)
			{
				//Run IDA
				dprintf("Analyzing [%s](%s)\n",TheSourceFilename,IDCFilename);
				Execute(TRUE,"\"%s\" -A -S\"%s\" \"%s\"",IDAPath,IDCFilename,TheSourceFilename);
				free(IDCFilename);
			}

			IDCFilename=WriteToTemporaryFile("static main()\n\
				{\n\
					Wait();\n\
					RunPlugin(\"DarunGrim2\",1);\n\
					SetLogFile(\"%s\");\n\
					...
					StartAddressForThePatched,
					EndAddressForThePatched
					);
			...
			{
				dprintf("Analyzing [%s](%s)\n",TheSourceFilename,IDCFilename);
				Execute(TRUE,"\"%s\" -A -S\"%s\" \"%s\"",IDAPath,IDCFilename,TheTargetFilename);
				//Delete IDC file
